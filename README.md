# 五子棋联机版 (Gomoku Multiplayer)

一款基于 Python + Tkinter + 原生 TCP Socket 的 Windows 桌面双人联机五子棋游戏。
支持多房间、断线检测、胜负/和棋判定、SQLite 对局记录、HTTP 管理页面、PyInstaller 打包成 EXE。

---

## 一、项目简介

本项目是一个完整的客户端 / 服务器（C/S）架构五子棋游戏：
...
| `client/ui.py` | `GomokuClientUI`：登录界面（服务器/端口/玩家/房间号）+ 游戏界面（玩家信息/回合/日志/按钮）+ 定时 `after(50ms)` 轮询 Queue |
...
1. 运行服务器（TCP 监听 8888，HTTP 监听 8080）。
2. 两个客户端打开登录界面，填写服务器地址、端口、昵称、相同房间号，点「加入房间」。
3. 第一个玩家拿到 `black`，第二个玩家拿到 `white`，服务器广播 `game_start`。
4. 任何一方点击棋盘，客户端发送 `{"type":"move","row":x,"col":y}` 到服务器。
5. 服务器检查：在房间？已开始？自己回合？坐标合法？空位置？→ 拒绝返回 `invalid_move`。
6. 合法则服务器落子 → 判断胜负/和棋 → 切换回合 → 广播 `move_result` 给两个客户端。
7. 若胜负决出：服务器广播 `game_over/draw`，并写入 SQLite（结束时间、胜者、完整 moves）。
8. 双方可随时点「重新开始」，服务器累计 2 票后重置棋盘并广播 `game_restart` + `game_start`。
9. 任一客户端断开，服务器从 Socket 读到 EOF，清理玩家并向对方广播 `player_left`，同时把未结束对局记为「对方胜」。
10. 管理面板每 3 秒刷新一次，管理员能看到全部房间/玩家/对局统计。

---

祝玩得愉快！如需扩展（AI 对战、观战模式、回放、账号系统），可在现有分层上继续增量开发。
